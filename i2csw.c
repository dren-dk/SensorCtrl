...
#define QDEL _delay_us(25);
#define HDEL _delay_us(100);
//#define QDEL _delay_ms(1);
//#define HDEL _delay_ms(1);
...
char i2cReceive(u08 device, u08 subAddr, u08 length, u08 *data)
{
    int j = length;
    u08 *p = data;
    ...
    I2C_STOP;    
    I2C_START;			                // do start transition
    if (!i2cPutbyte(device)) return 1;		// send DEVICE address
    if (!i2cPutbyte(subAddr)) return 2;   	// and the subaddress
    HDEL;
    I2C_SCL_HI;      		                // do a repeated START
    I2C_START;			                // transition
    
    if (!i2cPutbyte(device | READ)) return 3;	// resend DEVICE, with READ bit set
